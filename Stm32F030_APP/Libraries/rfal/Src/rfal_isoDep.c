...
#define RFAL_ISODEP_SPARAM_TVL_HDR_LEN  (2)                                                    /*!< S(PARAMETERS) TVL header lenght: Tag + Len */
...
ReturnCode rfalIsoDepPollHandleSParameters( rfalIsoDepDevice *isoDepDev, rfalBitRate maxTxBR, rfalBitRate maxRxBR )
{
    ...
    EXIT_ON_ERR( ret, rfalTransceiveBlockingTxRx( (uint8_t*)&sParam, (RFAL_ISODEP_SPARAM_HDR_LEN + it), (uint8_t*)&sParam, sizeof(rfalIsoDepControlMsgSParam), &rcvLen, RFAL_TXRX_FLAGS_DEFAULT, (isoDepDev->info.FWT + isoDepDev->info.dFWT) ));
    ...
    for( it=0; it<(rcvLen-RFAL_ISODEP_SPARAM_TAG_LEN); it++ )
    {
        if( (sParam.sParam.value[it] == RFAL_ISODEP_SPARAM_TAG_SUP_PCD2PICC) && (sParam.sParam.value[it+RFAL_ISODEP_SPARAM_TAG_LEN] == RFAL_ISODEP_SPARAM_TAG_PCD2PICC_LEN) )
        {
            supPCD2PICC = sParam.sParam.value[it + RFAL_ISODEP_SPARAM_TAG_PCD2PICC_LEN];
        }
        
        if( (sParam.sParam.value[it] == RFAL_ISODEP_SPARAM_TAG_SUP_PICC2PCD) && (sParam.sParam.value[it+RFAL_ISODEP_SPARAM_TAG_LEN] == RFAL_ISODEP_SPARAM_TAG_PICC2PCD_LEN) )
        {
    ...
    if( (supPICC2PCD == 0x00) || (supPCD2PICC == 0x00) )
    {
        return ERR_PROTO;
    }
    
    for( it=0; it<=maxTxBR; it++ )
    {
        if( supPCD2PICC & (0x01 << it) )
        {
            txBR = (rfalBitRate)it;
        }
    }
    for( it=0; it<=maxRxBR; it++ )
    {
        if( supPICC2PCD & (0x01 << it) )
        {
            rxBR = (rfalBitRate)it;
        }
    }
...
static void rfalIsoDepCalcBitRate( rfalBitRate maxAllowedBR, uint8_t piccBRCapability, rfalBitRate *dsi, rfalBitRate *dri )
{
    ...
    do
    {
        ...
        if( (RFAL_ISODEP_BITRATE_RFU_MASK & piccBRCapability) || (maxAllowedBR > RFAL_BR_848) )
        {
            return;
        }
