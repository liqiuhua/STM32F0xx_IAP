    ...
    *nfcId1Len   = 0;
    ...
    for( i = RFAL_NFCA_SEL_CASCADE_L1; i <= RFAL_NFCA_SEL_CASCADE_L3; i++)
    {
        ...
        do
        {
            ...
            if (ret == ERR_TIMEOUT 
                && backtrackCnt && !doBacktrack
                && !(RFAL_NFCA_SDD_REQ_LEN==bytesTxRx && 0==bitsTxRx))
            { 
                ...
                collBit = *((uint8_t*)&selReq + bytesTxRx) & (1 << bitsTxRx);
                collBit = ((0==collBit)?1:0); // invert the collision bit
                doBacktrack = true;
                backtrackCnt--;
            }
            else
            ...
            if( ret == ERR_RF_COLLISION )
            {
                ...
                if( (bytesTxRx + (bitsTxRx ? 1 : 0)) > (RFAL_NFCA_SDD_RES_LEN + RFAL_NFCA_SDD_REQ_LEN) )
                {
                    return ERR_PROTO;
                }

                if( (bytesTxRx + (bitsTxRx ? 1 : 0)) > (RFAL_NFCA_CASCADE_1_UID_LEN + RFAL_NFCA_SDD_REQ_LEN) && backtrackCnt )
                { /* Collision in BCC: Anticollide only UID part */
                    backtrackCnt--;
                    bytesTxRx = RFAL_NFCA_CASCADE_1_UID_LEN + RFAL_NFCA_SDD_REQ_LEN - 1;
                    bitsTxRx = 7;
                    collBit = *((uint8_t*)&selReq + bytesTxRx) & (1 << bitsTxRx); /* Not a real collision, extract the actual bit for the subsequent code */
                }
