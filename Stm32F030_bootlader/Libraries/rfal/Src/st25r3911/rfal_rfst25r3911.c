...
void ClearGT(void)     //此函数放在读写按键中，当初始化RF之后，会使gRFAL.tmr.GT参数不等于0，影响后面的寻卡，所以在寻卡之前把它清零
{
	  gRFAL.tmr.GT = 0;
}
...
{
    ReturnCode ret;
    static INT16U OutWhileFlag=10000;   //防止卡死在这个循环  
	
    do{
        rfalWorker(); 
			  OutWhileFlag--;
      }
    while( ((ret = rfalGetTransceiveStatus() ) == ERR_BUSY) && rfalIsTransceiveInTx() && OutWhileFlag);
...
{
    ReturnCode ret;
    static int OutWhileFlag=10000;

    do{
        rfalWorker();
			  OutWhileFlag--;
    }
    while( ((ret = rfalGetTransceiveStatus() ) == ERR_BUSY) && rfalIsTransceiveInRx() && OutWhileFlag);  
